-- Normalize hero poster order to natural one-based positions and keep future reorders contiguous.

with ordered as (
  select
    id,
    row_number() over (order by sort_order, created_at, id)::integer as natural_order
  from public.marketing_hero_posters
)
update public.marketing_hero_posters poster
set sort_order = ordered.natural_order
from ordered
where poster.id = ordered.id;
