...
set local role authenticated;
select public.save_onboarding_step(3, '{"referral_other_text":"Community event"}');
select public.save_onboarding_step(4, '{"interest_ids":["40000000-0000-0000-0000-000000000001","40000000-0000-0000-0000-000000000002"],"other_interest_text":"Robotics"}');
select test_security.assert((select onboarding_completed_at is not null and other_interest_text = 'Robotics' from public.mentee_profiles), 'completion persisted');
select test_security.assert((select count(*) = 2 from public.mentee_interests), 'multiple interests persisted');
select test_security.denied($q$select public.save_onboarding_step(4, jsonb_build_object('interest_ids', jsonb_build_array('40000000-0000-0000-0000-000000000001'), 'other_interest_text', repeat('x', 501)))$q$, 'atomic final failure after interest replacement');
