...
select test_private_mentoring_rules.assert((select count(*) >= 2 from public.commerce_items where item_kind='intensive_mentoring_package' and is_available),'fixed Intensive packages register as purchasable Commerce Items');
select test_private_mentoring_rules.assert((select bool_and(not r.is_available) from public.intensive_mentoring_packages p cross join lateral public.resolve_commerce_item(p.id) r where p.pricing_mode='consultation'),'consultation Intensive packages remain outside checkout');
...
select '99300000-0000-0000-0000-000000000001'::uuid,p.id,'private_mentoring','Private 3','private-3',p.price_amount from public.private_mentoring_packages p join public.mentor_tiers t on t.id=p.mentor_tier_id where t.code='TOP_STUDENT' and p.session_count=3
union all
select '99300000-0000-0000-0000-000000000001'::uuid,p.id,'private_mentoring','Private 5','private-5',p.price_amount from public.private_mentoring_packages p join public.mentor_tiers t on t.id=p.mentor_tier_id where t.code='TOP_STUDENT' and p.session_count=5
union all
(select '99300000-0000-0000-0000-000000000001'::uuid,p.id,'intensive_mentoring_package',p.name,p.slug,p.price_amount from public.intensive_mentoring_packages p where p.pricing_mode='fixed' and p.is_active order by p.sort_order,p.id limit 1);
...
insert into public.order_items(order_id,commerce_item_id,item_kind_snapshot,name_snapshot,slug_snapshot,unit_price_amount)
select '99300000-0000-0000-0000-000000000002'::uuid,p.id,'private_mentoring','Private 3 repeat','private-3-repeat',p.price_amount from public.private_mentoring_packages p join public.mentor_tiers t on t.id=p.mentor_tier_id where t.code='TOP_STUDENT' and p.session_count=3;
...
select public.submit_private_mentoring_topic_request((select s.id from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id=auth.uid() and e.purchased_sessions=5 and s.session_number=1),(select id from public.private_mentoring_session_focuses where code='IDEA_PROBLEM_FRAMING'),'Saya ingin membahas validasi problem dan mempersempit target pengguna.');
select test_private_mentoring_rules.assert((select s.topic_status='pending_review' and s.status='awaiting_focus' from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id=auth.uid() and e.purchased_sessions=5 and s.session_number=1),'mentee request requires admin review and does not auto-approve focus');
...
select public.admin_resolve_private_mentoring_session_topic((select s.id from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id='99100000-0000-0000-0000-000000000002' and e.purchased_sessions=5 and s.session_number=1),(select id from public.private_mentoring_session_focuses where code='IDEA_PROBLEM_FRAMING'),'Validasi problem statement dan ICP utama','Arahkan mentee ke satu hipotesis utama sebelum eksperimen.');
...
select mentor_id,date_trunc('week',local_start)::date,extract(isodow from local_start)::integer,'08:00','18:00' from (values
...
select set_config('request.jwt.claim.sub','99100000-0000-0000-0000-000000000002',true);
select public.submit_private_mentoring_topic_request((select s.id from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id=auth.uid() and e.purchased_sessions=5 and s.session_number=1),null,'Saya ingin menggeser sesi ke review pitch deck yang sudah direvisi.');
select test_private_mentoring_rules.assert((select s.status='scheduled' and s.topic_status='pending_review' from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id=auth.uid() and e.purchased_sessions=5 and s.session_number=1),'topic re-review does not destroy upcoming schedule');
select set_config('request.jwt.claim.sub','99100000-0000-0000-0000-000000000001',true);
select public.admin_resolve_private_mentoring_session_topic((select s.id from public.private_mentoring_sessions s join public.private_mentoring_enrollments e on e.id=s.enrollment_id where e.mentee_id='99100000-0000-0000-0000-000000000002' and e.purchased_sessions=5 and s.session_number=1),(select id from public.private_mentoring_session_focuses where code='SLIDE_DECK_VISUAL_DESIGN'),'Review pitch deck final dan narrative flow','Fokuskan feedback pada storyline dan slide problem-solution.');
reset role;
set local role service_role;
