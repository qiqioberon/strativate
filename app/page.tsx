...
export function Dashboard({ role, credits = 0, go, notify }: { role: 'mentee' | 'mentor' | 'admin'; credits?: number; go: (v: string) => void; notify: (m: string) => void }) { return <main className="dashboard-page"><aside className="dash-side"><div className="brand"><span className="brand-mark">S</span><span>strativate</span></div><p className="side-label">Workspace</p>{['Overview', 'My programs', 'Mentors', 'Resources'].map((item, i) => <button className={i === 0 ? 'side-active' : ''} key={item}><LayoutDashboard size={16} />{item}</button>)}<div className="side-bottom"><button><ChevronDown size={15} /> Settings</button><div className="profile-mini"><span className="avatar">M</span><span><strong>Marsha K.</strong><small>Mentee account</small></span></div></div></aside><div className="dash-content"><div className="dash-top"><div><p className="kicker">Tuesday, 17 August 2026</p><h1>Good morning, Marsha.</h1></div><button className="button button-outline" onClick={() => { window.location.href = '/' }}>Back to site</button></div>{role === 'mentee' ? <MenteeContent credits={credits} go={go} notify={notify} /> : role === 'mentor' ? <MentorContent notify={notify} /> : <AdminContent />}</div></main> }
