...
export default function Page() {
  ...
  function signIn() { setSignedIn(true); setView('dashboard'); notify('Welcome back, future winner.') }
    ...
    <div className="site-shell">
      <header className="site-header">
        ...
        <div className="header-actions">
          {signedIn ? <button className="avatar-button" onClick={() => go('dashboard')}><span className="avatar">M</span><span className="desktop-only">My dashboard</span><ChevronDown size={15} /></button> : <button className="text-button desktop-only" onClick={signIn}>Sign in</button>}
      ...
      {view === 'faq' && <Faq />}
      {view === 'dashboard' && <Dashboard credits={credits} go={go} notify={notify} />}
...
function Dashboard({ credits, go, notify }: { credits: number; go: (v: string) => void; notify: (m: string) => void }) { const [role, setRole] = useState('mentee'); return <main className="dashboard-page"><aside className="dash-side"><div className="brand"><span className="brand-mark">S</span><span>strativate</span></div><p className="side-label">Workspace</p>{['Overview', 'My programs', 'Mentors', 'Resources'].map((item, i) => <button className={i === 0 ? 'side-active' : ''} key={item}><LayoutDashboard size={16} />{item}</button>)}<div className="side-bottom"><button><ChevronDown size={15} /> Settings</button><div className="profile-mini"><span className="avatar">M</span><span><strong>Marsha K.</strong><small>Mentee account</small></span></div></div></aside><div className="dash-content"><div className="dash-top"><div><p className="kicker">Tuesday, 17 August 2026</p><h1>Good morning, Marsha.</h1></div><div className="role-switch"><button className={role === 'mentee' ? 'selected' : ''} onClick={() => setRole('mentee')}>Mentee</button><button className={role === 'mentor' ? 'selected' : ''} onClick={() => setRole('mentor')}>Mentor</button><button className={role === 'admin' ? 'selected' : ''} onClick={() => setRole('admin')}>Admin</button></div></div>{role === 'mentee' ? <MenteeContent credits={credits} go={go} notify={notify} /> : role === 'mentor' ? <MentorContent notify={notify} /> : <AdminContent />}</div></main> }
