    ...
    <div className="site-shell">
      <header className="site-header">
        ...
        <div className="header-actions">
          ...
          <button className="button button-primary desktop-only" onClick={() => { window.location.href = '/explore' }}>Mulai belajar <ArrowRight size={16} /></button>
...
function Home({ go, buy }: { go: (v: string) => void; buy: (v: string) => void }) {
  return <main>
    <section className="hero"><div className="hero-copy"><div className="eyebrow"><Sparkles size={15} /> Bekal unggul dalam kompetisi</div><h1>Raih kemenangan.<br /><em>Melangkah lebih jauh.</em></h1><p className="hero-lede">Bimbingan personal dan persiapan kompetisi bersama para juara, konsultan, dan profesional muda berpengalaman.</p><div className="hero-actions"><button className="button button-primary" onClick={() => go('programs')}>Temukan programmu <ArrowRight size={17} /></button><button className="button button-ghost" onClick={() => go('mentors')}><CirclePlay size={17} /> Kenali mentor kami</button></div><div className="hero-note"><span className="mini-stack"><i>AH</i><i>NP</i><i>RA</i></span><span><strong>2.500+</strong> peserta telah mengembangkan keunggulannya</span></div></div><div className="hero-art"><div className="art-note note-one">Finalis nasional <Trophy size={15} /></div><div className="art-card art-main"><div className="portrait portrait-orange"><span>AH</span></div><div><p className="small-label">Kenali mentor</p><h3>Alvin Haryanto</h3><p>Strategi & pemecahan kasus</p></div><div className="rating"><Star size={14} fill="currentColor" /> 4,9</div></div><div className="art-card art-side"><Award size={23} /><strong>15+ kemenangan</strong><span>di 4 negara</span></div><div className="art-scribble">bekal<br /><b>unggulmu</b></div></div></section>
    ...
    <section className="section programs-preview"><div className="section-head"><div><p className="kicker">Pilih bekal unggulmu</p><h2>Satu tujuan. <em>Caramu sendiri.</em></h2></div><button className="arrow-link" onClick={() => go('programs')}>Lihat semua program <ArrowRight size={16} /></button></div><div className="program-grid">{programs.map((p) => <ProgramCard key={p.title} program={p} />)}</div></section>
    <section className="quote-section"><div className="quote-mark">“</div><blockquote>Bersama Strativate, persiapanmu memiliki arah yang jelas.</blockquote><p>— Dibangun oleh para juara kompetisi untuk generasi juara berikutnya.</p><button className="button button-dark" onClick={() => buy("Panduan awal gratis")}>Dapatkan panduan awal gratis <ArrowRight size={16} /></button></section>
    <section className="section mentor-preview"><div className="section-head"><div><p className="kicker">Sosok di balik keunggulanmu</p><h2>Belajar dari mereka<br /><em>yang sudah membuktikannya.</em></h2></div><button className="arrow-link" onClick={() => go('mentors')}>Kenali semua mentor <ArrowRight size={16} /></button></div><div className="mentor-grid">{mentors.map((m) => <MentorCard mentor={m} key={m.name} />)}</div></section>
  </main>
}
...
function Programs() {
  return <main className="page-main"><PageIntro kicker="Temukan jalurmu" title={<>Program untuk<br /><em>kemenangan berikutnya.</em></>} text="Bandingkan sesi fleksibel dan bimbingan terstruktur untuk menemukan dukungan yang sesuai dengan persiapanmu." />
...
function About({ go }: { go: (v: string) => void }) { return <main className="page-main about-page"><PageIntro kicker="Mengapa Strativate hadir" title={<>Keunggulan bukan bawaan.<br /><em>Keunggulan dibangun.</em></>} text="Kami membantu pelajar dan mahasiswa mengubah keraguan menjadi langkah nyata menuju kemenangan." /><div className="about-grid"><div className="about-big"><span className="kicker">Keyakinan kami</span><h2>Bakat membuka kesempatan. <em>Persiapan membuatmu berkesan.</em></h2></div><div className="about-copy"><p>Kompetisi menguji lebih dari jawaban akhir: cara merumuskan masalah, mengambil keputusan, dan mengajak orang lain mendukung idemu.</p><p>Strativate menyediakan mentor, kerangka berpikir, dan ruang latihan agar peserta dapat mempersiapkan diri dengan terarah.</p><button className="button button-primary" onClick={() => go('mentors')}>Kenali komunitas kami <ArrowRight size={16} /></button></div></div><div className="values"><div><Globe2 /><strong>Mengutamakan peserta</strong><p>Ambisimu menentukan langkah.</p></div><div><Mic2 /><strong>Praktis</strong><p>Belajar melalui praktik nyata.</p></div><div><MessageCircle /><strong>Jujur</strong><p>Masukan yang membantu kamu berkembang.</p></div></div></main> }
