...
function Resources() { return <Management title="Pustaka materi" items={['Kerangka analisis kasus · PDF · Termasuk dalam Private Mentoring', 'Daftar periksa presentasi · Templat · Termasuk dalam Kelas Besar', 'Catatan estimasi ukuran pasar · Video · Sudah dibeli']} action="Tambah materi" /> }
function Management({ title, items, action }: { title: string; items: string[]; action: string }) { return <><Title eyebrow="Operasional produk" title={`${title}.`} detail="Buat, edit, arsipkan, pratinjau, dan tambahkan materi belajar ke program yang tepat." /><section className="role-card management-list"><div className="role-card-heading"><h2>Katalog yang dipublikasikan</h2><button className="button button-primary">{action}</button></div>{items.map((item) => <div className="service-row" key={item}><div><strong>{item.split(' · ')[0]}</strong><p>{item.split(' · ').slice(1).join(' · ')}</p></div><button className="text-link">Ubah <ChevronRight /></button></div>)}</section></> }
