...
type EnrollmentRow = {
  ...
  mentor_tier_name: string
  purchased_sessions: number
  awaiting_focus_sessions: number
  awaiting_scheduling_sessions: number
...
export function PrivateMentoringSessionManagement() {
  ...
  const [mentors, setMentors] = useState<EligiblePrivateMentoringMentor[]>([])
  ...
  const loadCatalog = useCallback(async () => {
    const [packageResult, tierResult] = await Promise.all([
    ...
    if (packageResult.error || tierResult.error) {
      setError('Filter paket Private Mentoring belum dapat dimuat.')
      return
    }
  ...
  const loadEnrollments = useCallback(async () => {
    setLoading(true)
    setError('')
    const { data, error: loadError } = await rpcClient.rpc<EnrollmentRow[]>('list_admin_private_mentoring_enrollments_page', {
      p_query: query.trim(),
    ...
    if (loadError) {
      setError('Daftar pesanan Private Mentoring belum dapat dimuat.')
      setLoading(false)
      return
    }
    const rows = data ?? []
    ...
    setEnrollments(rows)
    setSelectedEnrollment(current => current ? (rows.find(row => row.enrollment_id === current.enrollment_id) ?? null) : null)
  ...
  const loadEnrollmentSessions = useCallback(async (enrollmentId: string, preferredSessionId = '') => {
    ...
    if (sessionError) {
      setError('Detail sesi belum dapat dimuat.')
      setSessions([])
    ...
    const rows = data ?? []
    setSessions(rows)
    const next = rows.find(row => row.session_id === preferredSessionId)
  ...
  useEffect(() => {
    const timer = setTimeout(() => { void loadEnrollments() }, 250)
  ...
  useEffect(() => {
    ...
    if (!dialog) return
    if (selectedEnrollment && !dialog.open) dialog.showModal()
    if (!selectedEnrollment && dialog.open) dialog.close()
  }, [selectedEnrollment])
  ...
  useEffect(() => {
    ...
    setStartAt(selectedSession.scheduled_start_at ? localDateTimeValue(selectedSession.scheduled_start_at) : '')
    setMessage('')
    setError('')
    void rpcClient.rpc<EligiblePrivateMentoringMentor[]>('list_eligible_private_mentoring_mentors', { p_session_id: selectedSession.session_id }).then(({ data, error: mentorError }) => {
      setMentors(mentorError ? [] : (data ?? []))
    })
  ...
  async function openEnrollment(row: EnrollmentRow) {
    setSelectedEnrollment(row)
    setSessions([])
    setSelectedSessionId('')
    setMessage('')
    setError('')
    await loadEnrollmentSessions(row.enrollment_id)
  }

  async function refreshSelected(preferredSessionId = selectedSessionId) {
    if (!selectedEnrollment) return
    await Promise.all([
      loadEnrollmentSessions(selectedEnrollment.enrollment_id, preferredSessionId),
      loadEnrollments(),
    ])
  }

  async function schedule() {
    if (!selectedSession || !mentorId || !startAt) {
      setError('Pilih mentor dan jadwal untuk sesi ini.')
      return
    }
    setBusy(true)
    setError('')
    setMessage('')
    const { error: scheduleError } = await rpcClient.rpc('admin_schedule_private_mentoring_session', {
    ...
    setMessage(`Jadwal sesi ${selectedSession.session_number} tersimpan.`)
    await refreshSelected(selectedSession.session_id)
    setBusy(false)
  }
  ...
  async function complete() {
    ...
    setError('')
    setMessage('')
    const { error: completeError } = await rpcClient.rpc('admin_set_private_mentoring_session_status', {
      p_session_id: selectedSession.session_id,
      p_status: 'completed',
    })
    if (completeError) {
    ...
    setMessage(`Sesi ${selectedSession.session_number} ditandai selesai.`)
    await refreshSelected(selectedSession.session_id)
    setBusy(false)
  }
  ...
  return <div className="ops-page mentoring-admin-page mentoring-enrollment-page">
    ...
    <div className="ops-filter-bar mentoring-enrollment-filters">
      <label className="ops-field ops-field--wide"><span>Cari user</span><div className="ops-input-with-icon"><Search aria-hidden="true" size={15} /><input type="search" value={query} onChange={event => { setQuery(event.target.value); setPage(0) }} placeholder="Username, nama, email, atau paket" /></div></label>
      ...
      <label className="ops-field"><span>Progress</span><select value={progress} onChange={event => { setProgress(event.target.value); setPage(0) }}><option value="all">Semua progress</option><option value="needs_focus">Menunggu fokus</option><option value="needs_scheduling">Perlu dijadwalkan</option><option value="configured">Semua sesi diatur</option><option value="in_progress">Sedang berjalan</option><option value="completed">Selesai</option></select></label>
      ...
      <label className="ops-field"><span>Per halaman</span><select value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(0) }}><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option></select></label>
      <button className="button button-outline" type="button" onClick={resetFilters}>Reset</button>
    </div>
    ...
    <section className="role-card ops-table-section">
      ...
      <div className="ops-table-wrap"><table className="ops-table mentoring-enrollment-table"><thead><tr><th>User</th><th>Email</th><th>Paket</th><th>Tanggal beli</th><th>Progress</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
        {enrollments.map(row => { const state = enrollmentStatus(row); return <tr key={row.enrollment_id}>
          <td><strong>{row.mentee_name}</strong><small className="ops-table-secondary">@{row.mentee_username || 'username-belum-diatur'}</small></td>
          <td>{row.mentee_email}</td>
          <td><strong>{row.package_name}</strong><small className="ops-table-secondary">{row.mentor_tier_name} · {row.purchased_sessions} sesi</small></td>
          <td>{DATE.format(new Date(row.purchased_at))}</td>
          <td><strong>{row.configured_sessions}/{row.purchased_sessions} diatur</strong><small className="ops-table-secondary">{row.completed_sessions} selesai · {row.awaiting_focus_sessions} tunggu fokus</small></td>
          <td><span className={`ops-status ops-status--${state.tone}`}>{state.label}</span></td>
          <td><button type="button" className="button button-outline" onClick={() => void openEnrollment(row)}>Kelola</button></td>
        </tr>})}
      ...
      {loading ? <p className="muted" role="status">Memuat pesanan mentoring…</p> : null}
      <TablePagination page={page} pageSize={pageSize} totalItems={total} onPageChange={setPage} disabled={loading} label="Pagination pesanan mentoring" />
    </section>
    ...
    <dialog ref={dialogRef} className="ops-dialog mentoring-enrollment-dialog" aria-labelledby="mentoring-enrollment-dialog-title" onClose={() => setSelectedEnrollment(null)} onClick={event => { if (event.target === event.currentTarget) event.currentTarget.close() }}>
      {selectedEnrollment ? <div className="ops-dialog__surface">
        ...
        {selectedSession ? <>
          <section className="mentoring-form-section"><div className="mentoring-form-section__title"><UserRound aria-hidden="true" /><div><span>Mentor Assignment</span><h4>Mentor untuk sesi {selectedSession.session_number}</h4></div></div><div className="mentoring-form-grid"><label className="ops-field"><span>Eligible mentor</span><select value={mentorId} onChange={event => setMentorId(event.target.value)} disabled={!selectedSession.session_focus_id || selectedSession.status === 'completed'}><option value="">Pilih mentor</option>{mentors.map(mentor => <option value={mentor.mentor_id} key={mentor.mentor_id}>{mentor.mentor_name}</option>)}</select></label><div className="ops-readonly-field"><span>Mentor tier wajib</span><strong>{selectedSession.mentor_tier_name}</strong><small>Daftar mentor otomatis dibatasi ke tier paket.</small></div></div></section>
          <section className="mentoring-form-section"><div className="mentoring-form-section__title"><CalendarDays aria-hidden="true" /><div><span>Schedule</span><h4>Jadwal sesi {selectedSession.session_number}</h4></div></div><div className="mentoring-form-grid"><label className="ops-field"><span>Date & start time</span><input type="datetime-local" value={startAt} onChange={event => setStartAt(event.target.value)} disabled={!selectedSession.session_focus_id || selectedSession.status === 'completed'} /></label><div className="ops-readonly-field"><span>Duration</span><strong>{durationLabel(selectedSession)}</strong><small>Durasi mengikuti entitlement paket.</small></div>{selectedSession.scheduled_start_at ? <div className="ops-readonly-field mentoring-form-grid__wide"><span>Current schedule</span><strong>{DATE_TIME.format(new Date(selectedSession.scheduled_start_at))}</strong></div> : null}</div></section>
          <section className="mentoring-form-section"><div className="mentoring-form-section__title"><Clock3 aria-hidden="true" /><div><span>Status / Action</span><h4>{selectedStatus?.label}</h4></div></div><div className="mentoring-form-actions"><div><span>Session {selectedSession.session_number}/{selectedEnrollment.purchased_sessions}</span><strong>{selectedSession.focus_name ?? 'Menunggu fokus mentee'}</strong></div><div className="button-row"><button className="button button-primary" type="button" onClick={() => void schedule()} disabled={busy || !selectedSession.session_focus_id || selectedSession.status === 'completed'}>{selectedSession.status === 'scheduled' ? 'Simpan perubahan jadwal' : 'Jadwalkan sesi'}</button>{selectedSession.status === 'scheduled' ? <button className="button button-outline" type="button" onClick={() => void complete()} disabled={busy}><CheckCircle2 aria-hidden="true" size={16} />Tandai selesai</button> : null}</div></div></section>
        </> : <p className="muted">Belum ada sesi pada paket ini.</p>}
