...
export function PeopleManagement({ role }: { role: 'mentor' | 'mentee' }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    } catch { setError('Undangan belum dapat dikirim. Coba lagi.') } finally { setBusy(false) }
  }
  return <section className="role-card admin-data-panel"><h2>{role === 'mentor' ? 'Mentor Management' : 'Mentees'}</h2>
    {role === 'mentor' && <form className="auth-form" onSubmit={invite}><label>Email mentor<input name="email" type="email" required maxLength={254} /></label><button className="button button-primary" disabled={busy}>{busy ? 'Mengirim…' : 'Invite Mentor'}</button></form>}
