...
export function PeopleManagement({ role }: { role: 'mentor' | 'mentee' }) {
  ...
  return <section className="role-card admin-data-panel"><h2>{role === 'mentor' ? "Kelola Mentor" : "Peserta"}</h2>
    {role === 'mentor' && <MentorInviteForm onInvited={() => { setInvitationRefresh(value => value + 1); return load() }} />}
    {error && <p role="alert" className="form-error">{error}</p>}
    {role === 'mentor' && <MentorInvitations refreshKey={invitationRefresh} onDeleted={() => { void load() }} />}
