...
export function TablePagination({ page, pageSize, totalItems, onPageChange, disabled = false, label = 'Pagination' }: Props) {
  ...
  return <nav className={styles.pagination} aria-label={label} data-testid="table-pagination">
    <button
      type="button"
      className={`button button-outline ${styles.edgeButton}`}
      disabled={disabled || currentPage === 0}
      onClick={() => onPageChange(currentPage - 1)}
      aria-label="Halaman sebelumnya"
    ><ChevronLeft size={14} aria-hidden="true" /><span>Sebelumnya</span></button>
    ...
    ><span>Berikutnya</span><ChevronRight size={14} aria-hidden="true" /></button>
