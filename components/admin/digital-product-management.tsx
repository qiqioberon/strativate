...
const emptyDraft: Draft = { name: '', slug: '', description: '', price: '', contentType: '', isPublished: false, homepageFeatured: false, homepageOrder: '0' }
...
export function DigitalProductManagement() {
  ...
  useEffect(() => {
    ...
    setDraft({ name:selected.name, slug:selected.slug, description:selected.description, price:String(selected.price_amount), contentType:selected.content_type ?? '', isPublished:selected.is_published, homepageFeatured:selected.homepage_featured, homepageOrder:String(selected.homepage_featured_order) })
  ...
  const editor = (creating || selected) ? <form className={styles.form} onSubmit={save} noValidate aria-busy={busy} data-testid={creating ? 'digital-product-create-mode' : 'digital-product-edit-mode'}>
    ...
    <section className={styles.formSection} aria-labelledby="digital-product-homepage-heading"><div className={styles.sectionHeading}><h3 id="digital-product-homepage-heading">Showcase beranda</h3><p>Pilih produk yang masuk ke animasi Card Swap pada beranda. Hanya produk yang sudah Published yang benar-benar tampil ke pengunjung.</p></div><label className={styles.publishToggle}><input data-testid="digital-product-homepage-featured" type="checkbox" checked={draft.homepageFeatured} onChange={event => setDraft(current => ({ ...current, homepageFeatured:event.target.checked }))} /><span><strong>Tampilkan di Card Swap beranda</strong><small>Admin dapat mengganti pilihan kapan saja tanpa mengubah kode website.</small></span></label><label className={styles.field}>Urutan Card Swap<input data-testid="digital-product-homepage-order" type="number" min={0} max={9999} step={1} value={draft.homepageOrder} disabled={!draft.homepageFeatured} onChange={event => setDraft(current => ({ ...current, homepageOrder:event.target.value }))} /><small className={styles.helper}>Angka lebih kecil tampil lebih depan. Nilai 0 menjadi prioritas tertinggi.</small></label></section>
  ...
  return <section className={dataStyles.page} data-testid="digital-product-management" aria-busy={busy}>
    ...
    <div className={dataStyles.surface} data-testid="digital-product-list-surface">
      ...
      {filteredProducts.length === 0 ? <div className={dataStyles.empty}>Tidak ada Digital Product yang sesuai dengan pencarian.</div> : <div className={dataStyles.tableScroll} data-testid="digital-product-table-scroll"><table className={`${dataStyles.table} ${dataStyles.productTable}`} data-testid="digital-product-table"><thead><tr><SortableTableHeader label="Produk" sortKey="product" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Jenis" sortKey="type" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Harga" sortKey="price" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Status" sortKey="status" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Beranda" sortKey="homepage" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Materi terlindungi" sortKey="content" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><SortableTableHeader label="Diperbarui" sortKey="updated_at" activeKey={sortKey} direction={sortDirection} onSortChange={changeSort} /><th scope="col" className={dataStyles.actionCell}>Aksi</th></tr></thead><tbody>{pagedProducts.map(product => {
        ...
        return <tr key={product.id} data-testid="digital-product-row"><td><div className={styles.tableProductIdentity}><div className={styles.tableThumbnail}><Image src={coverUrl} alt="" fill sizes="54px" unoptimized /></div><div className={dataStyles.identityText}><strong className={dataStyles.primaryText}>{product.name}</strong><span className={dataStyles.descriptionText}>/{product.slug}</span></div></div></td><td><span className={`${dataStyles.badge} ${product.content_type ? dataStyles.successBadge : dataStyles.warningBadge}`}>{product.content_type ? product.content_type.toUpperCase() : 'Belum diatur'}</span></td><td><strong className={dataStyles.primaryText}>{formatDigitalProductPrice(product.price_amount)}</strong></td><td><span className={`${dataStyles.badge} ${product.is_published ? dataStyles.successBadge : dataStyles.warningBadge}`}>{product.is_published ? 'Published' : 'Draft'}</span></td><td><span className={`${dataStyles.badge} ${product.homepage_featured ? dataStyles.successBadge : dataStyles.warningBadge}`}>{product.homepage_featured ? `Card Swap #${product.homepage_featured_order}` : 'Tidak'}</span></td><td><span className={`${dataStyles.badge} ${contentReady ? dataStyles.successBadge : dataStyles.warningBadge}`}>{contentReady ? 'Siap' : 'Belum ada'}</span></td><td><time className={dataStyles.dateCell} dateTime={product.updated_at}>{formatUpdatedAt(product.updated_at)}</time></td><td className={dataStyles.actionCell}><div className={styles.tableActions}>{product.is_published ? <a className={`button button-outline ${dataStyles.actionButton}`} href={`/produk-digital/${product.slug}`} target="_blank" rel="noreferrer">Preview</a> : null}<button type="button" className={`button button-outline ${dataStyles.actionButton}`} onClick={() => beginEdit(product.id)} data-testid={`digital-product-manage-${product.id}`}>Kelola</button></div></td></tr>
      })}</tbody></table></div>}
