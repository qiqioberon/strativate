...
import { MentorPublicationControl } from './mentor-publication-control'
...
export function MentorManagement() {
  ...
  const loadTiers = useCallback(async () => {
    try {
      const { data, error: loadError } = await createClient().from('mentor_tiers').select('*').eq('is_active', true).order('sort_order').order('name')
  ...
  function changeSort(key: string | null, direction: SortDirection) {
    setSortKey(key as MentorSortKey | null)
  ...
  async function setMentorActive(mentor: ManagedMentor, isActive: boolean) {
    ...
    setError('')
    try {
      const { error: actionError } = await createClient().rpc('set_mentor_active', {
  ...
  async function deleteMentor(mentor: ManagedMentor) {
    ...
    setError('')
    try {
      const { error: deleteError } = await createClient().rpc('delete_mentor_account', { p_mentor_id: mentor.user_id })
