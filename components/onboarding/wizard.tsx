...
export function OnboardingWizard({ profile, mentee, names, referrals, interests, initialInterests, initialInstitution }: Props) {
  ...
  const [interestOther, setInterestOther] = useState(mentee.other_interest_text || ''), [otherSelected, setOtherSelected] = useState(!!mentee.other_interest_text)
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = step === 1 ? profilePayload({ firstName, lastName, username, password, confirmation, passwordRequired })
      ...
      : interestPayload({ interestIds: selectedInterests, otherText: otherSelected ? interestOther : '' })
  ...
  return <><div className="auth-heading"><p className="kicker">Langkah {step} dari 4</p><h1>{steps[step - 1]}<em>.</em></h1><p>Lengkapi profil untuk memulai perjalananmu. Progres tersimpan setiap kali kamu melanjutkan.</p></div>
    ...
    <form className="auth-form" onSubmit={submit}><fieldset disabled={busy}>
      {step === 1 && <><label>Nama Depan<input value={firstName} onChange={e => setFirstName(e.target.value)} required maxLength={100} autoComplete="given-name" /></label><label>Nama Belakang<input value={lastName} onChange={e => setLastName(e.target.value)} maxLength={100} autoComplete="family-name" /></label><label>Username<input value={username} onChange={e => setUsername(e.target.value)} required minLength={3} maxLength={30} autoComplete="username" /></label><label>Password{!passwordRequired && ' (opsional)'}<input type="password" value={password} onChange={e => setPassword(e.target.value)} required={passwordRequired} minLength={8} maxLength={128} autoComplete="new-password" /></label><label>Konfirmasi Password<input type="password" value={confirmation} onChange={e => setConfirmation(e.target.value)} required={passwordRequired || !!password} autoComplete="new-password" /></label>{passwordSaved && <p role="status">Password akun sudah tersimpan.</p>}{profile.registration_method === 'google' && <p>Kamu tetap dapat masuk dengan Google tanpa membuat password.</p>}</>}
