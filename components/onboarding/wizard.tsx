...
export function OnboardingWizard({ profile, mentee, names, referrals, interests, initialInterests, initialInstitution }: Props) {
  ...
  return <><div className="auth-heading"><p className="kicker">Langkah {step} dari 4</p><h1>{steps[step - 1]}<em>.</em></h1><p>Lengkapi profil untuk memulai perjalananmu. Progres tersimpan setiap kali kamu melanjutkan.</p></div>
    ...
    <form className="auth-form" onSubmit={submit}><fieldset disabled={busy}>
      {step === 1 && <><label>Nama Depan<input value={firstName} onChange={e => setFirstName(e.target.value)} required maxLength={100} autoComplete="given-name" /></label><label>Nama Belakang<input value={lastName} onChange={e => setLastName(e.target.value)} maxLength={100} autoComplete="family-name" /></label><label>Username<input value={username} onChange={e => setUsername(e.target.value)} required minLength={3} maxLength={30} autoComplete="username" /></label><PasswordInput label={passwordRequired ? 'Password' : 'Password (opsional)'} value={password} onChange={e => setPassword(e.target.value)} required={passwordRequired} minLength={8} maxLength={128} autoComplete="new-password" /><PasswordInput label="Konfirmasi Password" value={confirmation} onChange={e => setConfirmation(e.target.value)} required={passwordRequired || !!password} autoComplete="new-password" />{passwordSaved && <p role="status">Password akun sudah tersimpan.</p>}{profile.registration_method === 'google' && <p>Kamu tetap dapat masuk dengan Google tanpa membuat password.</p>}</>}
      ...
      {step === 4 && <><p>Pilih satu atau lebih minat kompetisimu.</p>{interests.map(option => <label className="option-label" key={option.id}><input type="checkbox" checked={selectedInterests.includes(option.id)} onChange={e => setSelectedInterests(ids => e.target.checked ? [...ids, option.id] : ids.filter(id => id !== option.id))} />{option.name}</label>)}</>}
