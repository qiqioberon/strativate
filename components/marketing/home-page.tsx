      ...
      {digitalProductsEnabled ? <section className="marketing-section marketing-products" aria-labelledby="product-heading" data-reveal data-testid="homepage-products-section">
        <div className="marketing-container marketing-products__grid">
          <div className="marketing-products__intro">
            ...
            <Link className={buttonVariants({ variant: 'outline', size: 'marketing' })} href="/produk-digital" data-testid="digital-products-link">Lihat Produk Digital <ArrowRight data-icon="arrow" size={16} /></Link>
          </div>
          <div className="marketing-product-stack">
            {digitalProducts.slice(0, 2).map((product, index) => <article className="marketing-product-card" key={product.id}>
              <div className="marketing-product-card__cover"><Image src={product.imageUrl} alt={`Sampul ${product.name}`} fill sizes="(max-width: 760px) 32vw, 13vw" unoptimized /></div>
              <div><span>Produk Digital · {formatRupiah(product.price_amount)}</span><h3>{product.name}</h3><p>{product.description}</p></div><strong aria-hidden="true">0{index + 1}</strong>
            </article>)}
