...
export function RoleCalendar({ role, onOpenAvailability }: { role: Role; onOpenAvailability?: () => void }) {
  ...
  return <div className="native-calendar">
    <div className="role-page-title">
      ...
      <p>{role === 'admin' ? 'Pantau seluruh sesi Strativate dan status Google Meet tanpa membuka detail kalender pribadi mentor atau mentee.' : 'Gabungkan sesi Strativate dengan agenda Google Calendar pribadi Anda. Jadwal mentoring tetap read-only.'}</p>
    </div>
    ...
    <dialog ref={dialogRef} className="calendar-dialog" onCancel={event => { event.preventDefault(); setSelected(null) }} onClose={() => setSelected(null)}>
      {selected ? <>
        ...
        <div className="calendar-dialog__actions calendar-dialog__actions--wrap">{selected.source === 'google' && selected.htmlLink ? <a className="button button-primary" href={selected.htmlLink} target="_blank" rel="noopener noreferrer"><ExternalLink />Buka di Google Calendar</a> : null}{selected.source === 'strativate' && selected.meetingUrl ? <><a className="button button-primary" href={selected.meetingUrl} target="_blank" rel="noopener noreferrer"><ExternalLink />Join Google Meet</a><button className="button button-outline" type="button" onClick={() => void navigator.clipboard.writeText(selected.meetingUrl!)}><Clipboard />Copy Meet Link</button></> : null}{selected.source === 'strativate' && role !== 'admin' ? <a className="button button-outline" href={supportHref(selected)} target="_blank" rel="noopener noreferrer">Hubungi Admin via WhatsApp</a> : null}{role === 'mentor' && selected.source === 'strativate' && onOpenAvailability ? <button className="button button-outline" type="button" onClick={() => { setSelected(null); onOpenAvailability() }}><UsersRound />Atur availability</button> : null}{role === 'admin' && selected.source === 'strativate' ? <><button className="button button-outline" type="button" onClick={() => { setScheduleId(selected.sessionId!); setSelected(null) }}>Reschedule</button>{selected.googleSyncStatus === 'failed' ? <button className="button button-outline" type="button" onClick={() => void retrySync(selected)}><RefreshCw />Retry Google Sync</button> : null}</> : null}</div>
        {role === 'admin' && selected.source === 'strativate' ? <div className="meeting-override"><label><span>Edit meeting link (override)</span><input type="url" placeholder="https://…" value={meetingDraft} onChange={event => setMeetingDraft(event.target.value)} /></label><div className="button-row"><button className="button button-outline" type="button" onClick={() => void saveMeeting(selected, meetingDraft.trim() || null)}><Link2 />Simpan link</button><button className="button button-ghost" type="button" disabled={!selected.manualMeetingUrl} onClick={() => void saveMeeting(selected, null)}>Reset ke Google Meet</button></div></div> : null}
      </> : null}
    </dialog>
