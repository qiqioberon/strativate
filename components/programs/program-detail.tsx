...
function PrivatePackages() {
  return <>
    <p className="program-pricing-note">Setiap sesi berlangsung 75 menit. Harga paket yang sama berlaku untuk individu maupun tim berisi 1–4 peserta.</p>
    <div className="program-comparison-grid">
      {privateMentorTiers.map(tier => <article className="program-price-table" key={tier.name}>
        ...
        <p className="program-table-note">Jadwal fleksibel · Pilih mentormu</p>
      </article>)}
    </div>
  </>
}
...
function IntensivePackages() {
  return <div className="program-three-grid">
    ...
    <article className="program-info-card program-package program-custom-package">
      ...
      <p className="program-package-price program-consultation-label">Melalui konsultasi</p>
...
function IntensiveExtras() {
  return <>
    <section id="add-ons" className="program-section">
      <div className="program-section-heading"><p className="kicker">Pilihan layanan tambahan</p><h2>Dukungan tambahan sesuai kebutuhanmu.</h2><p>Lengkapi paket utamamu dengan evaluasi tambahan, simulasi, atau perlindungan berdasarkan capaian.</p></div>
    ...
    <section className="program-section" aria-labelledby="bundle-heading">
      <div className="program-section-heading"><p className="kicker">Dukungan terpadu</p><h2 id="bundle-heading">Paket yang dirancang sesuai tujuanmu.</h2><p>Bandingkan layanan yang disertakan dalam setiap paket gabungan mentoring.</p></div>
...
export function ProgramInformationDetail({ program }: { program: ProgramInformation }) {
  ...
  return <main className="detail-page program-information">
    ...
    <section className="program-audience"><Compass size={30} aria-hidden="true" /><div><p className="kicker">Apakah ini untukmu?</p><h2>Mulai dari tahapmu saat ini.</h2><p>{program.audience}</p></div></section>

    {isPrivate && <>
      <section className="program-section"><div className="program-section-heading"><p className="kicker">Dua jalur belajar</p><h2>Perkuat dasarmu. Fokus pada targetmu.</h2></div><div className="program-comparison-grid">{privateLearningPaths.map(path => <article className="program-info-card" key={path.title}><h3>{path.title}</h3><p>{path.description}</p></article>)}</div></section>
      <section className="program-section"><div className="program-section-heading"><p className="kicker">Fokus sesi</p><h2>Kerjakan prioritas utamamu.</h2><p>Fokus pada satu prioritas setiap sesi untuk mendapatkan masukan yang jelas dan langkah lanjutan yang dapat diterapkan. Topik khusus dapat dibahas saat konsultasi.</p></div><div className="program-three-grid">{privateTopics.map(topic => <article className="program-info-card" key={topic.title}><h3>{topic.title}</h3><p>{topic.description}</p></article>)}</div></section>
    </>}

    <section className="program-section"><div className="program-section-heading"><p className="kicker">Manfaat yang kamu dapatkan</p><h2>{isPrivate ? 'Sesi terarah. Wawasan praktis.' : 'Bimbingan konsisten. Perkembangan nyata.'}</h2></div><FeatureList items={program.outcomes} /></section>
    <section className="program-section"><div className="program-section-heading"><p className="kicker">Cara kerja program</p><h2>Perjalanan mentoringmu.</h2></div><ol className="program-journey">{program.journey.map((step, index) => <li key={step.title}><span className="program-step-number" aria-hidden="true">0{index + 1}</span><div><h3>{step.title}</h3><p>{step.description}</p></div></li>)}</ol></section>
