...
export function ProfileForm() {
  ...
  return <section className="workspace-card account-profile"><h2>Profil akun</h2><form className="auth-form" onSubmit={submit}><label>Nama Depan<input name="first_name" defaultValue={profile.first_name || ''} required maxLength={100} /></label><label>Nama Belakang<input name="last_name" defaultValue={profile.last_name || ''} maxLength={100} /></label><label>Nama pengguna<input name="username" defaultValue={profile.username || ''} required maxLength={30} /></label><label>URL Foto Profil<input name="avatar_url" type="url" defaultValue={profile.avatar_url || ''} maxLength={2048} placeholder="https://…" /></label>{error && <p className="form-error" role="alert">{error}</p>}{saved && <p role="status">Profil tersimpan.</p>}<button className="button button-primary" disabled={busy}>{busy ? 'Menyimpan…' : 'Simpan profil'}</button></form></section>
}
