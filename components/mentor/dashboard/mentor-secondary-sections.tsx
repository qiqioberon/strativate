...
export function MentorProfilePanel({ data, open }: { data: MentorDashboardData; open: (section: MentorDashboardSection) => void }) {
  return <div className="mentor-section"><MentorPageHeader eyebrow="Profil" title="Akun dan informasi operasional mentor." detail="Data akun dapat Anda edit sesuai policy profil; tier dan status mentor berasal dari domain mentor canonical." action={<button type="button" className="button button-primary" onClick={() => open('availability')}>Atur ketersediaan</button>}/>{data.metadataError ? <p className="mentor-inline-warning" role="status">{data.metadataError}</p> : null}<div className="mentor-profile-grid"><ProfileForm/><section className="workspace-card mentor-operational-profile"><div><p className="kicker">Informasi mentor</p><h2>Status operasional</h2><p>Informasi berikut read-only dan mengikuti konfigurasi domain mentor.</p></div><dl><div><dt>Tier mentor</dt><dd>{data.tierName || 'Belum ditetapkan'}</dd></div><div><dt>Timezone</dt><dd>{data.timezone}</dd></div><div><dt>Status akun mentor</dt><dd><span className={`ops-status ops-status--${data.isActive ? 'positive' : 'danger'}`}>{data.isActive ? 'Aktif' : 'Nonaktif'}</span></dd></div><div><dt>Ketersediaan minggu ini</dt><dd><span className={`ops-status ops-status--${availabilityTone(data.availability.current)}`}>{availabilityLabel(data.availability.current)}</span></dd></div><div><dt>Ketersediaan minggu depan</dt><dd><span className={`ops-status ops-status--${availabilityTone(data.availability.next)}`}>{availabilityLabel(data.availability.next)}</span></dd></div></dl></section></div></div>
}
