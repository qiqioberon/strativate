...
export function MenteePanel({ data, open, onRetry }: { data: MentorDashboardData; open: (section: MentorDashboardSection) => void; onRetry: () => void }) {
  ...
  return <div className="mentor-section">
    <MentorPageHeader eyebrow="Peserta saya" title="Peserta yang Anda dampingi." detail="Setiap baris mengikuti enrollment Private Mentoring yang memiliki sesi yang benar-benar dialokasikan kepada Anda." action={<button type="button" className="button button-primary" onClick={() => open('availability')}>Atur ketersediaan</button>}/>
