...
export function SessionDetailDialog({ session, timezone, onClose }: { session: MentorSessionRow | null; timezone: string; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="ops-dialog mentor-detail-dialog" aria-labelledby="mentor-session-detail-title" onClose={onClose}>
    {session ? <div className="ops-dialog__surface">
      ...
      <div className="ops-detail-grid"><div><span>Status</span><strong>{mentorSessionStatusLabel(session.status)}</strong></div><div><span>Fokus</span><strong>{session.focus_name || 'Belum dicatat'}</strong></div><div><span>Paket</span><strong>Private Mentoring · {session.purchased_sessions} sesi</strong></div><div><span>Durasi</span><strong>{session.duration_minutes ? `${session.duration_minutes} menit` : '—'}</strong></div></div>
      ...
      {session.status === 'scheduled' && session.meeting_url ? <a className="button button-primary" href={session.meeting_url} target="_blank" rel="noopener noreferrer">Join Google Meet <ExternalLink aria-hidden="true"/></a> : null}
    </div> : null}
  </dialog>
}
...
export function MenteeDetailDialog({ summary, timezone, onClose }: { summary: MentorMenteeSummary | null; timezone: string; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="ops-dialog mentor-detail-dialog" aria-labelledby="mentor-mentee-detail-title" onClose={onClose}>
    {summary ? <div className="ops-dialog__surface">
      ...
      <section className="ops-dialog__section"><h3>Fokus mentoring</h3><p>{summary.focusNames.length ? summary.focusNames.join(', ') : 'Belum ada fokus yang tercatat pada sesi yang dialokasikan kepada Anda.'}</p></section>
      <section className="ops-dialog__section"><h3>Sesi yang dialokasikan kepada Anda</h3><div className="mentor-dialog-session-list">{summary.sessions.map(session => <article key={session.session_id}><div><strong>Sesi {session.session_number}/{session.purchased_sessions}</strong><span>{session.focus_name || 'Fokus belum dicatat'} · {sessionDate(session, timezone)}</span></div><span className={statusClass(session.status)}>{mentorSessionStatusLabel(session.status)}</span>{session.status === 'scheduled' && session.meeting_url ? <a href={session.meeting_url} target="_blank" rel="noopener noreferrer" aria-label={`Join Google Meet sesi ${session.session_number}`}><ExternalLink aria-hidden="true"/></a> : null}</article>)}</div></section>
    </div> : null}
  </dialog>
}
