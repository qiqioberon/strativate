...
export function MenteeDetailDialog({ summary, timezone, onClose }: { summary: MentorMenteeSummary | null; timezone: string; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="ops-dialog mentor-detail-dialog" aria-labelledby="mentor-mentee-detail-title" onClose={onClose}>
    {summary ? <div className="ops-dialog__surface">
      ...
      <div className="ops-detail-grid"><div><span>Program</span><strong>{summary.programName}</strong></div><div><span>Sesi dialokasikan</span><strong>{summary.assignedSessions}</strong></div><div><span>Selesai</span><strong>{summary.completedSessions}/{summary.progressSessions}</strong></div><div><span>Sesi berikutnya</span><strong>{summary.nextSession ? sessionDate(summary.nextSession, timezone) : 'Belum ada'}</strong></div></div>
      ...
      <section className="ops-dialog__section"><h3>Sesi yang dialokasikan kepada Anda</h3><div className="mentor-dialog-session-list">{summary.sessions.map(session => {const topic=session as TopicAwareMentorSession;return <article key={session.session_id}><div><strong>Sesi {session.session_number}{session.purchased_sessions?'/'+session.purchased_sessions:''}</strong><span>{topic.resolved_topic || session.focus_name || 'Topik belum dicatat'} · {sessionDate(session, timezone)}</span><small>Session ID: {session.session_id}</small>{topic.mentor_scope_notes?<small>{topic.mentor_scope_notes}</small>:null}</div><span className={statusClass(session.status)}>{mentorSessionStatusLabel(session.status)}</span>{session.status === 'scheduled' && session.meeting_url ? <div className="mentor-meeting-actions"><a href={session.meeting_url} target="_blank" rel="noopener noreferrer" aria-label={`Join Zoom sesi ${session.session_number}`}><ExternalLink aria-hidden="true"/></a><CopyTextButton value={session.meeting_url} label="Salin link Zoom" copiedLabel="Link disalin"/></div> : null}</article>})}</div></section>
    </div> : null}
  </dialog>
}
