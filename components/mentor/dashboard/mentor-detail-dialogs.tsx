...
export function MenteeDetailDialog({ summary, timezone, onClose }: { summary: MentorMenteeSummary | null; timezone: string; onClose: () => void }) {
  ...
  return <dialog ref={ref} className="ops-dialog mentor-detail-dialog" aria-labelledby="mentor-mentee-detail-title" onClose={onClose}>
    {summary ? <div className="ops-dialog__surface">
      ...
      <section className="ops-dialog__section"><h3>Sesi yang dialokasikan kepada Anda</h3><div className="mentor-dialog-session-list">{summary.sessions.map(session => {const topic=session as TopicAwareMentorSession;return <article key={session.session_id}><div><strong>Sesi {session.session_number}/{session.purchased_sessions}</strong><span>{topic.resolved_topic || session.focus_name || 'Topik belum dicatat'} · {sessionDate(session, timezone)}</span>{topic.mentor_scope_notes?<small>{topic.mentor_scope_notes}</small>:null}</div><span className={statusClass(session.status)}>{mentorSessionStatusLabel(session.status)}</span>{session.status === 'scheduled' && session.meeting_url ? <a href={session.meeting_url} target="_blank" rel="noopener noreferrer" aria-label={`Join Google Meet sesi ${session.session_number}`}><ExternalLink aria-hidden="true"/></a> : null}</article>})}</div></section>
    </div> : null}
  </dialog>
}
