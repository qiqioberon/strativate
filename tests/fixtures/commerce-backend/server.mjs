...
const profileFor = id => ({ id, role: id === ids.admin ? 'admin' : id === ids.mentor ? 'mentor' : 'mentee', first_name: id === ids.mentor ? 'Mentor' : id === ids.admin ? 'Admin' : 'Mentee', last_name: 'Fixture', username: null, avatar_url: null, registration_method: 'email', mentor_setup_completed_at: id === ids.mentor ? now : null, password_set_at: now, created_at: now, updated_at: now })
...
function bearerUser(req) { const authorization = req.headers.authorization || ''; const token = authorization.replace(/^Bearer\s+/i, ''); let id = ids.mentee; try { const payload = JSON.parse(Buffer.from(token.split('.')[1] || '', 'base64url').toString('utf8')); if ([ids.mentee, ids.mentor, ids.admin].includes(payload.sub)) id = payload.sub } catch {} return { id, aud: 'authenticated', role: 'authenticated', email: id === ids.mentor ? 'mentor@example.test' : id === ids.admin ? 'admin@example.test' : 'mentee@example.test', email_confirmed_at: now, phone: '', app_metadata: { provider: 'email', providers: ['email'] }, user_metadata: {}, identities: [], created_at: now, updated_at: now } }
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/rest/v1/mentee_profiles' && req.method === 'GET') { const requested = filterEq(url, 'user_id'); return postgrest(req, res, requested === ids.mentee ? [menteeProfile] : []) }
