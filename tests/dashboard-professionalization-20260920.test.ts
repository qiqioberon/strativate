...
test('notification bell is unread-only while history keeps read filters and structured deep-links',()=>{
  ...
  assert.match(history,/notificationCategoryTypes/)
  assert.doesNotMatch(history,/const CATEGORY_TYPES/)
...
test('mentor assignments expose canonical Session ID search and keep copy actions inside detail',()=>{
...
test('Zoom creation explicitly requests cloud recording and PATCH does not rewrite auto recording',()=>{
  ...
  assert.doesNotMatch(update,/auto_recording\s*:/)
