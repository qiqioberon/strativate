...
test('mobile calendar uses a deterministic grid for navigation and view controls',()=>{
...
test('disconnected calendar explains Google 403 test-user setup before leaving Strativate',()=>{
 const calendar=read('components/calendar/role-calendar.tsx')
 assert.match(calendar,/calendar-connection-help/)
 assert.match(calendar,/403 access_denied/)
 assert.match(calendar,/Audience.*Test users/)
 assert.match(calendar,/calendar-oauth-notice/)
})
