...
test('testimonial gallery continuously moves, pauses on hover, and opens details in a modal', () => {
  ...
  assert.match(gallery, /selected\.originalImageUrl/)
...
test('admin testimonial manager uploads to Supabase Storage and controls publish/order state', () => {
  ...
  assert.match(admin, /testimonials\/original\//)
  assert.match(admin, /testimonials\/gallery\//)
  assert.match(admin, /original_image_path/)
  assert.match(admin, /contentType: selectedFile\.type/)
