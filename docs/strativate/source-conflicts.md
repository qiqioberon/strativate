...
## 21 September 2026 — approved mentor website spreadsheet import

The stakeholder-supplied `Data Mentor for website.xlsx` is approved as the public editorial source for its 26 mentor rows. This decision supersedes the older guardrail against database seeding from the earlier frontend roster, only for facts normalized from this approved workbook. All rows are intended for current website publication because the workbook is explicitly the website roster and contains no separate draft/publication column.

The normalized, reviewable source is `supabase/seed/mentor_website_profiles.json`; it stores a SHA-256 normalized-email identity hash instead of plaintext email or private workbook notes. During the current development phase, `scripts/seed-mentor-website.ts` reads plaintext email only from the local approved workbook, verifies every row against that committed hash, then creates a missing Auth account or reuses an existing mentor/mentee account. It configures the operational mentor tier and public profile and sets the same temporary password from the uncommitted `MENTOR_SEED_SHARED_PASSWORD` environment variable. Production execution is blocked and the destructive development action requires `ALLOW_DEV_MENTOR_ACCOUNT_SEED=true`; `MENTOR_SEED_ALLOWED_SUPABASE_URL` must also exactly match the configured development project URL. Admin accounts are never overwritten.

The earlier profile-only bulk importer is retired by the forward rollback migration `202609210003_rollback_mentor_website_seed_import.sql`; already-created public profiles are preserved. The replacement service-only RPC in `202609210004_dev_mentor_account_seed.sql` updates only the requested mentor, preserves mentor-managed fields absent from the workbook and an existing public slug, and additively deduplicates achievements/expertise. If profile configuration fails after creating a new Auth user, the command removes that new user to avoid an orphan. Existing approved portrait asset keys are reused; the seven documented missing-photo fallbacks remain unchanged. Public pages continue to read only published safe fields through `list_public_mentors()`.
...
| Mentor identities and proof | Public runtime reads only published account-owned public profiles from the database; the development-only 21 September seed creates or reuses Auth-backed mentor accounts and publishes no ratings. | `Data Mentor for website.xlsx` is the approved current website roster; its normalized committed seed omits plaintext email, private notes, credentials, and unsupported claims. | Run the guarded development seeder with its shared password supplied only through local environment configuration; production execution remains prohibited and ratings still require real system data. | Resolved/Guardrail |
