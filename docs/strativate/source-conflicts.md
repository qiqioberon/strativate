...
The normalized, reviewable source is `supabase/seed/mentor_website_profiles.json`; it stores a SHA-256 normalized-email identity hash instead of plaintext email or private workbook notes. `scripts/seed-mentor-website.ts` matches that identity only to an existing Auth user that already owns an operational `mentor_profiles` row. It never creates Auth users, credentials, or invitations. Unmatched accounts and operational-tier mismatches are reported for explicit admin action and are not published.

The import updates only matched spreadsheet profiles in one transactional bulk call, deterministically reconciles only child rows previously owned by this seed, preserves mentor-managed fields that are absent from the workbook, preserves an existing public slug, and does not delete mentor profiles absent from the workbook. Existing approved portrait asset keys are reused; the seven documented missing-photo fallbacks remain unchanged. Public pages continue to read only published safe fields through `list_public_mentors()`.
...
| Mentor identities and proof | Public runtime reads only published account-owned public profiles from the database; the approved 21 September website seed attaches editorial data only to existing real mentor accounts and publishes no ratings. | `Data Mentor for website.xlsx` is the approved current website roster; its normalized seed omits plaintext email, private notes, and unsupported claims. | Run the deterministic importer after account invitation/tier correction; unmatched rows remain unseeded and ratings still require real system data. | Resolved/Guardrail |
