...
`public/assets/placeholders/` and are represented by the typed registry at
`lib/content/asset-registry.ts`.

| Asset/content | Status | Expected source | Current fallback | Expected final file or record | Used in | Priority |
| --- | --- | --- | --- | --- | --- | --- |
| Primary horizontal logo | Missing — `REQUIRED` | Strativate SVG master + transparent PNG | `brand.logo.primary` → `public/assets/placeholders/logo-development.svg` | `public/assets/brand/strativate-logo-primary.svg` | Header, footer, auth, dashboards | P0 |
| Logo mark | Missing — `REQUIRED` | Strativate SVG master | `brand.logo.mark` → `public/assets/placeholders/logo-development.svg` | `public/assets/brand/strativate-logo-mark.svg` | Mobile header, favicon, sidebars, OG | P0 |
...
| Mentor portrait | Missing — `REQUIRED` | Original JPG/PNG + rights, ideally 1200×1200+ | `mentors.<id>.portrait` → `public/assets/placeholders/portrait-development.svg` | `public/assets/mentors/<mentor-id>/portrait.*` | Hero slot, cards, profile, dashboards | P0 |
...
| Big Class cover | Conditional | Strativate master or approved developer composition | `programs.bigClass.cover` → `public/assets/placeholders/media-development.svg` | `public/assets/programs/big-class/<program-id>/cover.*` | Catalog/detail/dashboard if visual | P1 |
...
| Digital Product cover | Missing — `REQUIRED` | Portrait master, ideally 1600×2400+ | `products.<id>.cover` → `public/assets/placeholders/cover-development.svg` | `public/assets/products/<product-id>/cover.*` | Home, catalog, library | P0 |
...
| Achievement image/certificate | Recommended if section is used | Original high-resolution file + rights | `achievements.featured.image` → `public/assets/placeholders/media-development.svg` | `public/assets/achievements/<achievement-id>/image.*` | Marketing/social proof | P1 |
