...
| Mentor roster and public profiles | Approved development account seed | `public.mentor_public_profiles`, `public.mentor_public_achievements`, `public.mentor_public_profile_expertise`, and `list_public_mentors()` own runtime public mentor data. The approved 26-row website workbook is normalized without plaintext email in `supabase/seed/mentor_website_profiles.json`; the guarded development seeder verifies local workbook emails against those hashes, creates or reuses Auth accounts, configures the mentor domain, and sets a temporary shared password from local environment configuration. `lib/content/mentors.ts` is not a production runtime source. | Apply migrations through `202609210004_dev_mentor_account_seed.sql`, configure the development-only environment values locally, run `pnpm seed:mentor-website -- --dry-run`, then run the seed without `--dry-run` after reviewing the counts |
...
| Intensive Mentoring commercial data | Database-backed + Shared Commerce integrated; migration required before deploy | Active fixed-price national packages, active add-ons (including Win Guarantee Protection), and valid active bundles (including Competition Assurance) register as Commerce Items. The public International Competition master stays consultation-only, while admin-created intended-mentee International Custom Offers register their own authoritative Commerce Items. Paid Intensive order items create normalized `intensive_mentoring_entitlements` and operational engagements. | Apply the latest Intensive commerce/operations forward migrations; maintain catalog activation/prices through Intensive admin CRUD and create negotiated International offers through Admin → Cart Links |
...
- The approved mentor website account seed requires migrations through `202609210004_dev_mentor_account_seed.sql`. Migration `202609210003_rollback_mentor_website_seed_import.sql` retires the previously applied profile-only importer without deleting profiles it already created. The replacement command is development-only, requires an explicit safety switch, an exact allowlisted development Supabase URL, and a 12+ character temporary password supplied through local environment configuration; it creates or reuses all verified workbook accounts and blocks protected/unsupported account conflicts before writes.
