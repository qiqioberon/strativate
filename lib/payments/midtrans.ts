...
function requiredString(value: unknown, field: string): string {
  if (typeof value !== 'string' || value.length === 0) throw new Error(`Midtrans response is missing ${field}.`)
  return value
}
...
export async function createMidtransSnapTransaction(input: {
  ...
  const response = await fetch(config.snapTransactionUrl, {
    method: 'POST',
    headers: {
...
export async function getMidtransTransactionStatus(providerOrderId: string): Promise<MidtransStatus> {
  ...
  const config = getConfig()
  const response = await fetch(`${config.statusBaseUrl}/${encodeURIComponent(providerOrderId)}/status`, {
    headers: {
      Accept: 'application/json',
      Authorization: authorization(config.serverKey),
    },
